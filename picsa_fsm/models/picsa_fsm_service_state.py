...
class PicsaFSMServiceState(models.Model):
    _name = "picsa.fsm.service.state"
    _description = "Estado de Servicio Realizado PICSA"
    _order = "sequence, id"
    ...
    _sql_constraints = [
        ("code_uniq", "unique(code)", "El código del estado debe ser único."),
    ]

    def _generate_code_from_name(self, name):
        code = re.sub(r"[^a-zA-Z0-9\s]", "", (name or "").lower())
        code = re.sub(r"\s+", "_", code.strip()) or "state"
        base_code = code
        counter = 1
